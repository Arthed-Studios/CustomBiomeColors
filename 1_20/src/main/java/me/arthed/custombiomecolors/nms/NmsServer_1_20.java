...
public class NmsServer_1_20 implements NmsServer {

    private final RegistryMaterials<BiomeBase> biomeRegistry = (RegistryMaterials<BiomeBase>) ((CraftServer) Bukkit.getServer()).getServer().aV().d(Registries.ap);

    @Override
    public NmsBiome getBiomeFromBiomeKey(BiomeKey biomeKey) {
        return new NmsBiome_1_20(this.biomeRegistry.a(ResourceKey.a(Registries.ap, new MinecraftKey(biomeKey.key, biomeKey.value))));
    }

    @Override
    public NmsBiome getBiomeFromBiomeBase(Object biomeBase) {
        return new NmsBiome_1_20(((Holder<BiomeBase>) biomeBase).a());
    }
    ...
    public void loadBiome(BiomeKey biomeKey, BiomeColors biomeColors) {
        ...
        Bukkit.getLogger().warning("LOADING: " + biomeKey.key + " - " + biomeKey.value);
